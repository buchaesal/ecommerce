...
public class CouponInfo {
    ...
    public void validateUsingCoupon(CouponRequest request){
        ...
        if(!isValidPeriod()){
            throw new RuntimeException("프로모션 기간이 아닙니다.");
        }
        if(!isValidMbrNo(request.getMbrNo())){
            throw new RuntimeException("쿠폰 사용 회원을 확인해주세요.");
        }
    }

    public void validateCancelCoupon(){
        if(!isUsedCoupon()){
            throw new RuntimeException("사용하지 않은 쿠폰입니다.");
        }
        if(hasNotUsedRestoredCoupon()){
            throw new RuntimeException("이미 취소 된 쿠폰입니다.");
        }
    }
